...
PORT = 1344 # change to the actual port
...
def faulty_and_table(rx, sy):
    r = get_random_bytes(16)
    table = []
    for i in range(4):
        sx = bitsToBytes([i // 2] * 16 * 8)
        ry = bitsToBytes([i % 2] * 16 * 8)
        table.append(xorBytes(r, xorBytes(xorBytes(rx, sx), xorBytes(sy, ry))).hex())

    return table, r

def faulty_and_gmw(conn, rx, sy):
    tab, r = faulty_and_table(rx, sy)
    for tr in tab:
        conn.sendline(tr.encode())
...
def main():
    ...
    t3 = and_gmw(conn, x3[0], x3[1])
    t4 = faulty_and_gmw(conn, x4[0], x4[1])
