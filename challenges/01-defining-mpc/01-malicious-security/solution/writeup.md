...
We can therefore send $p = p_1p_2 \dots p_n + 1$, where $p_1, p_2, \dots, p_n$ are the first $n$ primes, and such that $p \in \mathbb{P}$. This way, the order of $\mathbb{F}(p)$ will be $\phi(p) = p - 1$, which is easily factorizable.

We can then apply the Pohlig-Hellman Algorithm for Discrete Logarithm to get $x$ very fast, due to the Finite Field order.